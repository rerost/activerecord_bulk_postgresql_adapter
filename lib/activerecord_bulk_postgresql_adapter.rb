...
module ActiveRecord
  module ConnectionAdapters
    ...
    class BulkPostgreSQLAdaptor < ActiveRecord::ConnectionAdapters::PostgreSQLAdapter
      ...
      def preload_indexes(table_names)
        ...
        @__preload[:indexes] ||= table_names.map do |table_name|
          ...
          indexes = scope_result.map do |row|
            ...
            comment = row[4]
            valid = row[5]
            columns = decode_string_array(row[6]).map { |c| ActiveRecord::ConnectionAdapters::PostgreSQL::Utils.unquote_identifier(c.strip.gsub('""', '"')) }
